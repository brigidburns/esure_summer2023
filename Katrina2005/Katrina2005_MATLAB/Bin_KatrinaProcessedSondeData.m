%%% Bin_KatrinaProcessedSondeData.m %%%
%%% This script bins the sonde data from the files processed manually by
%%% ASPEN
%%% Saves to katrina2005_processed_data_rad.mat and
%%% katrina2005_processed_constants_rad.mat
clear all
close all
clc
...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        % 'read_katrina_processed_frd_files.m' to reduce reading time of each individual
        % sonde
        frd_filename = strcat('./Katrina2005_Processed/',hurr{1}(1:end-1),'_processed_frd_files.mat'); 
        load(frd_filename); 
...
%% Saving Variables
%  Script saves to katrina2005_processed_data_rad.mat and
%  katrina2005_processed_constants_rad.mat
save('katrina2005_processed_data_rad.mat','all_U_profiles','all_theta_profiles','all_q_profiles','all_k_profiles','all_p_profiles');
save('katrina2005_processed_constants_rad.mat','rho','cpa','cpl','cpv','Lv','pbl_height','max_height',...
