%%% Bin_KatrinaSondeData.m %%%
%%% This script bins only the sonde data from the original .frd files
%%% Saves to katrina2005_data_rad.mat and katrina2005_constants_rad.mat
clear all
close all
clc
...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        frd_filename = strcat('./Katrina2005_FRD/',hurr{1}(1:end-1),'_frd_files.mat');
        load(frd_filename);
...
%% Saving Variables
%  Saves to katrina2005_data_rad.mat and katrina2005_constants_rad.mat
save('katrina2005_data_rad.mat','all_U_profiles','all_theta_profiles','all_q_profiles','all_k_profiles','all_p_profiles');
save('katrina2005_constants_rad.mat','rho','cpa','cpl','cpv','Lv','pbl_height','max_height',...
