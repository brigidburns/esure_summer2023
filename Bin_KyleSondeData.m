...
% Trying to follow Bin_SondeData, but for only one storm
timeStart = BT_yyyymmddhhmm{1,304}; 
...
numObs = sum(~isnan(TCOBS_time_since_first_data_point(:,304))); 
for w=1:numObs % Cycle through TCOBS for that storm
    rmw_time_TC(w,304) = datenum([str2num(timeStart(1:4)) str2num(timeStart(5:6)) str2num(timeStart(7:8)) ...
        str2num(timeStart(9:10))+TCOBS_time_since_first_data_point(w,304) str2num(timeStart(11:12)) 00]); 
end
...
if (isEBT + isTCOBS > 0)
    if isTCOBS>0
        ...
        rmw_time_hurr = cach1(cach1>0&cach2>0); 
        rmw_rad_hurr  = cach2(cach1>0&cach2>0); 
