%%% Bin_SandySondeData.m %%%
%%% This script bins only the sonde data from the original .frd files
%%% Saves to sandy2012_data_rad.mat and sandy_constants_rad.mat
clear all
close all
clc
...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        frd_filename = strcat('./Sandy2012_FRD/',hurr{1}(1:end-1),'_frd_files.mat');
        load(frd_filename);
...
%% Saving Variables
%  Saves to sandy2012_data_rad.mat and sandy2012_constants_rad.mat
save('sandy2012_data_rad.mat','all_U_profiles','all_theta_profiles','all_q_profiles','all_k_profiles','all_p_profiles');
save('sandy2012_constants_rad.mat','rho','cpa','cpl','cpv','Lv','pbl_height','max_height',...
...
save('sandy2012_variables.mat','sondexvec','sondeyvec','max_rad'); 