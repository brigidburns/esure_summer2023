%%% Bin_SandyProcessedSondeData.m %%%
%%% This script bins only the sonde data from the .frd files processed by
%%% ASPEN
%%% Saves to sandy2012_processed_data_rad.mat and sandy_processed_constants_rad.mat
clear all
close all
clc

% Split into 10 m/s-wide bins from 0 to 80 m/s (each with its own mean
% profile) 
...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        % 'read_katrina_frd_files.m to reduce reading time of each individual sonde
        frd_filename = strcat('./Sandy2012_Processed/',hurr{1}(1:end-1),'_processed_frd_files.mat');
        load(frd_filename);
        ...
        for i=1:length(files) % 2. loop every dropsonde
            ...
            if (~isempty(z))  % Only do the computations if data is left after the clean
                ...
                sondexvec_processed(count_sonde) = x/r_rmw_rad_hurr;
                sondeyvec_processed(count_sonde) = y/r_rmw_rad_hurr;
...
figure(3)
hold on
plot(sondexvec_processed,sondeyvec_processed,'x')
xlabel('x')
ylabel('y')
...
%% Saving Variables
%  Saves to sandy2012_processed_data_rad.mat and sandy2012_processed_constants_rad.mat
save('sandy2012_processed_data_rad.mat','all_U_profiles','all_theta_profiles','all_q_profiles','all_k_profiles','all_p_profiles');
save('sandy2012_processed_constants_rad.mat','rho','cpa','cpl','cpv','Lv','pbl_height','max_height',...
...
save('sandy2012_processed_variables.mat','sondexvec_processed','sondeyvec_processed'); 