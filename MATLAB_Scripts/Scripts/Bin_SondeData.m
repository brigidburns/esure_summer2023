...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        frd_filename = strcat(filedir,hurr{1}(1:end-1),'_frd_files.mat');
        load(frd_filename);
...
%% Saving Variables
save('../Output_Files/combined_all_profiles_data_rad.mat','all_U_profiles','all_theta_profiles','all_q_profiles','all_k_profiles','all_p_profiles');
save('../Output_Files/combined_constants_rad.mat','rho','cpa','cpl','cpv','Lv','pbl_height','max_height',...
