...
all_processed_U_profiles = cell(num_z,num_rad_bins,num_wind_bins);
...
all_processed_p_profiles = cell(num_z,num_rad_bins,num_wind_bins);
all_processed_q_profiles = cell(num_z,num_rad_bins,num_wind_bins);
all_processed_k_profiles = cell(num_z,num_rad_bins,num_wind_bins);
all_processed_theta_profiles = cell(num_z,num_rad_bins,num_wind_bins);
...
% Without ones with no profiles used in the processing
 hurrvec = {'Katrina2005/' 'Kyle2008/' 'Sandy2012/' ...
    'Danny2015/' 'Ela2015/' 'Erika2015/' 'Guillermo2015/' 'Hilda2015/' 'Joaquin2015/' 'Kate2015/' 'Oho2015/' 'Patricia2015/' ...
    'Bonnie2016/' 'Hermine2016/' 'Javier2016/' 'Karl2016/' 'Matthew2016/' ...
...
for hurr = hurrvec % 1. loop every hurricane
    ...
    if (isEBT + isTCOBS > 0) %(~isnan(cach)) % ensure hurr name of dropsondes corresponds to a rmw_name from the track data
        ...
        frd_filename = strcat(filedir,hurr{1}(1:end-1),'_processed_frd_files.mat');
        load(frd_filename);
                ...
                if (radbin~=0 && ~isnan(radbin) && ~isnan(windbin))
                    ...
                    for j=1:length(z)
                        if (z(j)<max_height&&z(j)>min_height)
                            ...
                            all_processed_U_profiles{zidx,radbin,windbin} = [all_processed_U_profiles{zidx,radbin,windbin} WS(j)];
                            ...
                            all_processed_p_profiles{zidx,radbin,windbin} = [all_processed_p_profiles{zidx,radbin,windbin} p(j)];
                            all_processed_q_profiles{zidx,radbin,windbin} = [all_processed_q_profiles{zidx,radbin,windbin} q(j)];
                            all_processed_k_profiles{zidx,radbin,windbin} = [all_processed_k_profiles{zidx,radbin,windbin} k(j)];
                            all_processed_theta_profiles{zidx,radbin,windbin} = [all_processed_theta_profiles{zidx,radbin,windbin} theta(j)];
                        end
                    end % z-grid
...
figure(6)
hold on
num_samples = sum(numsonde(:,:),1);
plot(Uplot,num_samples,'-*k')

%% Saving Variables
save('../Output_Files/combined_all_processed_profiles_data_rad.mat','all_processed_U_profiles','all_processed_theta_profiles','all_processed_q_profiles','all_processed_k_profiles','all_processed_p_profiles');