...
load('../Output_Files/combined_all_dc_profiles_data_rad.mat')
...
zplot_dc = min_height+0.5*height_interval:height_interval:max_height-0.5*height_interval; 
num_z = length(zplot); 
...
for j=1:num_wind_bins % wind radius
    for i=1:num_rad_bins % bin radius
        ...
        if (length(ufit_dc) > min_fit_samples)
            Ucoeffs_dc(:,i,j) = polyfit(log(zfit_dc),ufit_dc',1); 
            test_dc = fit(log(zfit_dc)',ufit_dc,'poly1'); 
            U_ci_dc = confint(test_dc,0.95); 
        else
            Ucoeffs_dc(:,i,j) = ones(2,1)*NaN; 
            U_ci_dc = ones(2,2)*NaN; 
        end
        ...
        delta_CD_dc(i,j) = CD_dc(i,j)*sqrt(2*(delta_ustar_dc(i,j)/abs(ustar_dc(i,j)))^2 + 2*(delta_u10_dc(i,j)/abs(u10_dc(i,j)))^2); 
